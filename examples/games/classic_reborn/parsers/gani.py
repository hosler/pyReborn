...
class GaniManager:
    ...
    def get_sprite_surface(self, gani_name: str, sprite_id: int) -> Optional[pygame.Surface]:
        ...
        # Debug sprite lookup
        if not hasattr(self, '_logged_sprite_lookup'):
            print(f"Looking up sprite {sprite_id} in GANI {gani_name}")
            print(f"GANI has {len(gani.sprites)} sprite definitions")
            if sprite_id in gani.sprites:
                sprite = gani.sprites[sprite_id]
                print(f"Found sprite {sprite_id}: {sprite.image} at ({sprite.x}, {sprite.y}) size ({sprite.width}x{sprite.height})")
            self._logged_sprite_lookup = True
            
        # Ensure required images are loaded
        for image_type, image_name in gani.default_images.items():
            if image_name and image_name not in self.image_cache:
