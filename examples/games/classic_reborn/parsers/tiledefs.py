...
class TileDefs:
    ...
    # Classic Graal item types (custom additions)
    HEART = 30
    RUPEE = 31
    BOMB_ITEM = 32
    ARROW_ITEM = 33
    ...
    CUTTABLE_TILES = {GRASS, TALL_GRASS}  # Can be cut with sword
    PICKUP_TILES = {HEART, RUPEE, BOMB_ITEM, ARROW_ITEM, KEY, HEART_CONTAINER}  # Item pickups
    ...
    def is_chair(self, tile_id: int) -> bool:
        """Check if tile is a chair"""
        return self.get_tile_type(tile_id) == self.CHAIR
        
    def is_cuttable(self, tile_id: int) -> bool:
        """Check if tile can be cut with sword"""
        # Bushes can be cut with sword (they disappear when hit)
        if tile_id in self.BUSH_TILES:
            return True
        return self.get_tile_type(tile_id) in self.CUTTABLE_TILES
        
    def is_pickup(self, tile_id: int) -> bool:
        """Check if tile is an item pickup"""
        return self.get_tile_type(tile_id) in self.PICKUP_TILES
        
    def is_grass(self, tile_id: int) -> bool:
        """Check if tile is grass"""
        tile_type = self.get_tile_type(tile_id)
        return tile_type == self.GRASS or tile_type == self.TALL_GRASS
