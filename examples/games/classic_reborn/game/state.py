...
class GameState:
    ...
    def __init__(self, tile_defs=None):
        ...
        self.chest_respawn_times: Dict[Tuple[int, int], float] = {}  # Chest position to respawn time
        self.grabbed_tile_pos: Optional[Tuple[int, int]] = None
        self.local_player_id: int = 0  # ID of local player
        self.gmap_levels: Dict[str, Level] = {}  # GMAP segment levels
        ...
        # Movement
        self.move_speed = ClassicConstants.MOVE_SPEED  # Pull from constants
    ...
    def update_player(self, player):
        ...
        if hasattr(player, 'id'):
            # Replace the entire player object to ensure all properties are up to date
            self.players[player.id] = player
