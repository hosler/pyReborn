...
class BushHandler:
    ...
    def check_grabbable_at_position(self, level, tile_defs, x: float, y: float) -> Optional[Tuple[int, int]]:
        ...
        """
        # Check multiple points for better grab detection
        # This helps when the player is between tiles
        check_points = [
            (x, y),           # Center
            (x + 0.3, y),     # Right
            (x - 0.3, y),     # Left
        ...
        for check_x, check_y in check_points:
            ...
            if 0 <= tile_x < 64 and 0 <= tile_y < 64:
                ...
                # Check if it's a blocking tile that can be grabbed
                if tile_defs.is_blocking(tile_id):
                    # Allow grabbing any blocking tile for now
                    # This makes it easier to test grab mechanics
    ...
    def try_pickup_bush(self, level, tile_defs, player_x: float, player_y: float, direction: Direction) -> Optional[Tuple[int, int]]:
        ...
        if grabbable_pos:
            ...
            # Only pick up if it's actually a bush tile
            if self.is_bush_tile(tile_id):
                self.carrying_bush = True
                return (tile_x, tile_y)
            # For non-bush blocking tiles, return None (can't pick up)
