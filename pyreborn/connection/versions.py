"""
Client version definitions and protocol configurations for different Reborn versions.
"""
...
class ClientType(Enum):
    """Client connection types - these are the bit positions for (1 << position)"""
    TYPE_CLIENT = 0       # PLTYPE_CLIENT - older clients (1 << 0) = 1
    TYPE_CLIENT2 = 4      # PLTYPE_CLIENT2 - 2.19-2.21, 3-3.01 (1 << 4) = 16
    TYPE_CLIENT3 = 5      # PLTYPE_CLIENT3 - 2.22+, 5.x, 6.x (1 << 5) = 32
    TYPE_RC = 1           # PLTYPE_RC - Remote Control (1 << 1) = 2
    TYPE_RC2 = 6          # PLTYPE_RC2 - New RC (1 << 6) = 64
    TYPE_NPCSERVER = 2    # PLTYPE_NPCSERVER (1 << 2) = 4
    TYPE_NC = 3           # PLTYPE_NC (1 << 3) = 8
