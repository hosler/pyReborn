...
class RebornEncryption:
    ...
    def limit_from_type(self, compression_type: int):
        """Set limit based on compression type - matches GServer-v2 exactly"""
        # GServer-v2 limits: { 0x02, 0x0C, 0x04, 0x04, 0x06, 0x04 }
        if compression_type == CompressionType.UNCOMPRESSED:  # 0x02
            self.limit = 0x0C  # 12 iterations = 48 bytes max
        elif compression_type == CompressionType.ZLIB:       # 0x04  
            self.limit = 0x04  # 4 iterations = 16 bytes max
        elif compression_type == CompressionType.BZ2:        # 0x06
            self.limit = 0x04  # 4 iterations = 16 bytes max
