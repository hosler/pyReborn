...
class TextDataHandler:
    ...
    def _handle_board_text(self, text: str) -> Dict[str, Any]:
        ...
        if text.startswith("BOARD"):
            logger.debug(f"Parsing BOARD text: {text[:60]}...")
        
        parts = text.split(' ', 5)
        
        if len(parts) < 5:
            logger.warning(f"Invalid BOARD format: {text[:50]}...")
        ...
        try:
            ...
            logger.debug(f"   Parsed: x={x}, y={y}, width={width}, height={height}, data_len={len(tile_data)}")
